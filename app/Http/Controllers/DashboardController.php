...
class DashboardController extends Controller
{
    ...
    public function editSchedule($slug)
    {
        $schedules = ScheduleService::getSchedulesBySlug($slug);
        return Inertia::render('Dashboard/Schedule/Edit', [
            'schedules' => $schedules
        ]);
    }

    public function updateSchedule($slug, ScheduleRequest $request)
    {
        ScheduleService::updateSchedule($slug, $request->validated());
        return Redirect::route('dashboard.pmi')->with('success', 'Schedule created successfully');
    }
